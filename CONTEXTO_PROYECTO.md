...
- **`saldos_cliente(p_cliente_id)`** — devuelve por comercio: `saldo`, `pendiente`, `remanente`. El `pendiente` incluye tanto las solicitudes de premios de ESE comercio como la porción que le toca de solicitudes de premios **Generales** (comercio_id null), repartida entre los comercios con más remanente primero — mismo criterio que `canjear_premio`. Así la suma de remanentes por comercio siempre coincide con `tarjetas.puntos_remanentes`. Implementada en SQL puro (CTE + función ventana, sin loops) para evitar el riesgo de la primera versión en PL/pgSQL con arrays. **`crear_solicitud` reutiliza esta misma función** para validar el saldo de un premio de comercio puntual (antes tenía su propio cálculo, que no consideraba los Generales pendientes — el botón de canje no se bloqueaba correctamente).
