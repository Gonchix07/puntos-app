...
create table if not exists public.cargas (
  ...
  factura_numero text,
  factura_pesos numeric(14,2) check (factura_pesos is null or factura_pesos > 0), -- null en ajustes
  pesos_por_punto numeric(12,2) not null,     -- snapshot de la config al momento
  puntos numeric(14,2) not null,              -- puntos otorgados (con signo en ajustes)
  origen text not null default 'manual' check (origen in ('manual', 'api', 'ajuste')),
  motivo text,                                -- justificación (solo ajustes)
  usuario_email text,
  created_at timestamptz not null default now()
);
...
-- ---------- Ajuste de puntos (ingreso/egreso justificado, solo admin) ----------
-- Un ajuste es una carga con origen 'ajuste': puntos con signo, sin factura,
-- con motivo. Impacta en saldos_por_comercio y en la Auditoría.
...
--
--  Reparte lo pendiente de premios GENERALES (comercio_id null) entre
--  los comercios con más remanente disponible primero (mismo criterio
...
-- ============================================================
--  CAMPAÑAS (idéntico a supabase/migration_campanias.sql)
-- ============================================================
...
create table if not exists public.campanias (
  ...
  -- Días de la semana habilitados (0=domingo … 6=sábado, igual que extract(dow) y JS Date.getDay()).
  -- null o vacío = todos los días, sin restricción.
  dias_semana int[],
  -- Cuántas veces puede usarse por cliente en el período vigente
  periodicidad text not null default 'ilimitado'
  ...
  constraint campanias_dias_semana_valida check (
...
-- ---------- Registro de usos de campaña (para controlar la periodicidad) ----------
-- Cada vez que el sistema de facturación aplica el descuento de una campaña
-- en una venta, registra acá el uso. campanias_vigentes_cliente lo consulta
-- para dejar de ofrecer la campaña hasta que termine el período vigente.
create table if not exists public.campania_usos (
  ...
  local_id uuid references public.locales(id) on delete set null,  -- local donde se facturó (no confundir con comercios, que es del sistema de puntos)
  usuario_email text,
...
--  - Día de la semana: hoy debe estar en dias_semana (o sin restricción).
--  - Periodicidad: si ya se usó dentro del período vigente (día/semana/mes)
--    según campania_usos, deja de listarse hasta que termine ese período.
...
-- ============================================================
--  RPC: campañas del cliente para el PORTAL (informativa, distinta de
--  campanias_vigentes_cliente que usa el POS/facturación).
--  Devuelve TODAS las campañas asignadas al cliente que están dentro
--  de vigencia por fecha y cumplen el mínimo de puntos — incluidas las
--  que hoy no se pueden usar (día no habilitado o periodicidad ya
--  consumida) — para mostrarlas igual, deshabilitadas, con el motivo.
--  campanias_vigentes_cliente (la que consulta el POS) NO se toca.
-- ============================================================
...
-- ============================================================
--  RPC: registra el uso de una campaña (el sistema de facturación la
--  llama luego de aplicar el descuento en una venta). Vuelve a validar
--  la elegibilidad completa (vigencia, día, mínimo, destinatario y
--  periodicidad) antes de registrar, para no dejar pasar dos usos que
--  violen la periodicidad por una condición de carrera.
-- ============================================================
...
begin
  -- Bloquea la campaña para serializar registros concurrentes del mismo cliente
...
-- campania_usos: lectura para autenticados (auditoría); la inserción la hace
-- registrar_uso_campania (security definer). Sin update/delete -> prohibidas.
